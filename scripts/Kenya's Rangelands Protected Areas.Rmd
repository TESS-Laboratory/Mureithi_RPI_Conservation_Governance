...
# 1.Skepticism about RPI Fluctuations:
  # Do not fully trust year-to-year fluctuations in the Retail Price Index (RPI).
...
ggplot(Amboseli.RPI.Visual, aes(x = factor(Year), y = RPI_value)) +
  ...
  stat_summary(fun = mean, geom = "point", shape = 21, fill = "white", size = 2) + # Combined the mean line with points to make it easier to visualise
  labs(
    title = "RPI Distribution Over Years in Amboseli NP (Violin Plot)",
    x = "Year",
    y = "RPI Value"
...
ggplot(Amboseli.Mean, aes(x = Year, y = Mean_RPI)) +
  ...
  geom_point(color = "black", fill = "white", shape = 21, size = 2) +
  labs(
...
```{r}
## Visualising Mean RPI over AET.Others as a line 

AET.Others.Mean <- AET.Others.RPI.Visual %>%
  group_by(Year) %>%
  summarise(Mean_RPI = mean(RPI_value, na.rm = TRUE))

ggplot(AET.Others.Mean, aes(x = Year, y = Mean_RPI)) +
  geom_line(color = "darkgreen", linewidth = 0.8) +
  geom_point(color = "black", fill = "white", shape = 21, size = 2) +
  labs(
    title = "Mean RPI Over Time in AET.Others",
    x = "Year",
    y = "Mean RPI"
  ) +
  theme_beautiful()
...
```{r}
## Theil-Sen across AET.Others

  # Sorting data by "Year" & "ID"
AET.Others.RPI.Visual.Arranged <- AET.Others.RPI.Visual %>% arrange(ID, Year)

# Split data by pixel ID
AET.Others.Pixel.List <- AET.Others.RPI.Visual.Arranged %>% group_split(ID)

# Apply sens.slope to each pixel group
AET.Others.TheilSen <- map_dfr(AET.Others.Pixel.List, function(df) {
  if (sum(!is.na(df$RPI_value)) >= 3) {
    slope <- tryCatch(
      sens.slope(df$RPI_value)$estimates,
      error = function(e) NA_real_
  ...
  # Joining slope results back with area info
AET.Others.TheilSen <- AET.Others.TheilSen %>%
  mutate(Area = "AET.Others")

  # Getting overall summary per area
AET.Others.SummaryTrend <- AET.Others.TheilSen %>%
  summarise(
    MeanSlope = mean(Slope, na.rm = TRUE),
    MedianSlope = median(Slope, na.rm = TRUE),
    SD = sd(Slope, na.rm = TRUE),
...
All.Pixel.Slopes <- bind_rows(
  ...
  NgaiNdethia.TheilSen, SouthKitui.TheilSen, AET.Conservancies.TheilSen, AET.Others.TheilSen)
