...
# Step 1: Compute median slope per Area and reorder levels
area_order <- Primary.Pixel.Slopes %>%
  group_by(Area) %>%
  summarise(MedianSlope = median(Slope, na.rm = TRUE)) %>%
  arrange(desc(MedianSlope)) %>%
  pull(Area)

# Step 2: Apply factor level ordering to the dataset
Primary.Pixel.Slopes <- Primary.Pixel.Slopes %>%
  mutate(Area = factor(Area, levels = area_order))

# Step 3: Plot with reordered factor
ggplot(Primary.Pixel.Slopes, aes(x = Area, y = Slope, fill = Area)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = 0.1, outlier.shape = NA, color = "black") +
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkred") +
  labs(title = "RPI Pixel Trend Distributions in Key Areas (Sorted by Median Slope)",
       x = "Area", y = "Theil–Sen Slope") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  theme_beautiful()
```
