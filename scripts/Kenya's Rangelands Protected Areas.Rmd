...
simulate_rpi <- function(summary_df, area_name, n_pixels = 1000) {
  summary_df %>%
    rowwise() %>%
    mutate(
...
amboseli_data <- simulate_rpi(Amboseli.Summary, "Amboseli Park")
tsavo_e_data  <- simulate_rpi(TsavoEast.Summary, "Tsavo East")
tsavo_w_data  <- simulate_rpi(TsavoWest.Summary, "Tsavo West")
ranches_data  <- simulate_rpi(Tsavo.Ranches.Summary, "Tsavo Ranches")
settled_data  <- simulate_rpi(Tsavo.SettledAreas.Summary, "Tsavo Settled Areas")
allzones_data <- simulate_rpi(Tsavo.AllZones.Summary, "Tsavo All Zones")
conserv_data  <- simulate_rpi(AET.Conservancies.Summary, "AET Conservancies")
buffers_data  <- simulate_rpi(AET.Buffers.Summary, "AET Buffers")

# --- Combine all datasets ---
rpi_all <- bind_rows(
  amboseli_data, tsavo_e_data, tsavo_w_data, ranches_data, 
  settled_data, allzones_data, conserv_data, buffers_data
)

# --- Plot ridgelines ---
ggplot(rpi_all, aes(x = RPI, y = Area, fill = Area)) +
  geom_density_ridges(
    quantile_lines = TRUE,
