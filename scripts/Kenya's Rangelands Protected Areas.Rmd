...
pacman::p_load(sf, terra, tidyverse, tidyterra, colorspace, ggrepel, patchwork, tmap, trend, slider, forcats, viridis, scales, hexbin, ggh4x, scico)
...
```{r}
# Visualising the Community Conservancies & Group Ranches within GAE
...
tmap_arrange(Map.Amboseli.RPI.Mean, Map.TsavoEast.RPI.Mean,
             ...
             Map.AET.Buffers.RPI.Mean, Map.AET.AllZones.RPI.Mean,
             Map.Tsavo.Ranches.RPI.Mean, Map.Tsavo.SettledAreas.RPI.Mean,
             Map.Tsavo.AllZones.RPI.Mean, ncol = 3, nrow = 3)
```
...
ggplot(Annual.RPI.Trends, aes(x = Year, y = MeanRPI, color = Area)) +
  geom_line(linewidth = 1) +   # use linewidth here
  geom_point(size = 2) +       # still use size for points
...
Primary.Concern.Areas <- bind_rows(
  Amboseli.RPI.Visual, TsavoEast.RPI.Visual, 
  TsavoWest.RPI.Visual, Tsavo.Ranches.RPI.Visual, 
  Tsavo.SettledAreas.RPI.Visual, Tsavo.AllZones.RPI.Visual, 
  AET.Conservancies.RPI.Visual,AET.Buffers.RPI.Visual, AET.AllZones.RPI.Visual)
...
library(scico)       # scientific colour maps for MAE/RMSE
library(viridisLite) # viridis for R^2
library(patchwork)   # combine plots

# Helper to pull a layer by name pattern (case-insensitive), with a nice error if missing
get_layer <- function(r, pattern) {
  i <- grep(pattern, names(r), ignore.case = TRUE)
  if (length(i) == 0) stop(sprintf("Layer not found for pattern '%s'. Available: %s",
                                   pattern, paste(names(r), collapse = ", ")))
  r[[i[1]]]
}

# Extract layers (adjust patterns if your names differ)
mae  <- get_layer(Temporal.Performance.Kenya, "^MAE$")
rmse <- get_layer(Temporal.Performance.Kenya, "^RMSE$")
r2   <- get_layer(Temporal.Performance.Kenya, "^(R2|rsq|r\\.?squared)$")

# Common map layer
kenya_border <- geom_sf(
  data = Kenya.Admin.Boundary, fill = NA, color = "black", linewidth = 0.3
)

# MAE plot
p_mae <- ggplot() +
  geom_spatraster(data = mae) +
  scale_fill_scico(palette = "rocket", name = "MAE", na.value = "grey90") +
  kenya_border +
  labs(title = "MAE") +
...
# RMSE plot
p_rmse <- ggplot() +
  geom_spatraster(data = rmse) +
  scale_fill_scico(palette = "magma", name = "RMSE", na.value = "grey90") +
  kenya_border +
  labs(title = "RMSE") +
  theme_minimal(base_size = 13) +
  theme(legend.position = "bottom")
...
p_r2 <- ggplot() +
  ...
  scale_fill_viridis_c(name = expression(R^2), limits = c(0, 1), na.value = "grey90") +
  kenya_border +
...
(p_mae | p_rmse | p_r2) +
  plot_annotation(
    title = "Temporal Performance over Kenya",
    subtitle = "MAE (avg error), RMSE (penalises big errors), and R² (variance explained)",
    theme = theme(
      plot.title = element_text(face = "bold", hjust = 0.5),
      plot.subtitle = element_text(hjust = 0.5)
    )
