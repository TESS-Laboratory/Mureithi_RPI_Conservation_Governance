...
Kenya.Subregional.Admin.Boundary <- st_read("C:/BIOM4009 ResearchProject/DATA/Geospatial Data/R Use/2.Kenya-s-Rangelands-Protected-Areas_files/additional_data_layers_wgs84/kenya_subregional_admin2_boundary_wgs84.shp")
```
...
```{r}
# Focusing on Amboseli, Tsavo East & West National Parks (v1. - check line 862 for v2.)
...
ggplot() +
  ...
  geom_sf(data = PAs.Interest %>% filter(NAME %in% c("Tsavo East", "Tsavo West", "Amboseli", "Chyulu Hills")),
  ...
  geom_sf_text(data = PAs.Interest %>% filter(NAME %in% c("Tsavo East", "Tsavo West", "Amboseli", "Chyulu Hills")),
               aes(label = NAME), color = "black", size = 4, fontface = "bold") +
  labs(title = "GAE & TCA Locations", fill = "National Parks") +
  theme_beautiful()
```
...
```{r}
# Next is checking RPI for NPs of Interest for Year 2000 (v1. - check line 875 for v2.)
...
```{r}
# Next is checking RPI for NPs of Interest for Year 2022 (v1. - check line 890 for v2.)
...
```{r}
# Mean RPI per Year for Greater Amboseli Ecosystem - National Parks only (v1. - check line 933 for v2.)
```
...
ggplot()+
  ...
  geom_sf(data = SouthKitui) +
  labs(title = "GAE/TCA - National Parks & Reserves") +
  theme_beautiful()
```
...
ggplot()+
  ...
  geom_sf(data = Tawi_A) +
  labs(title = "Community & Group Ranches - GAE/TCA") +
  theme_beautiful()
```

```{r}
# Visualising the Community Conservancies & Group Ranches in GAE/TCA
...
ggplot() +
  ...
  geom_sf(data = NgaiNdethia, aes(fill = "Forest Areas"), color = 'black', linewidth = 0.1, 
          alpha = 0.5) +
  geom_sf(data = SouthKitui, aes(fill = "Forest Areas"), color = 'black', linewidth = 0.1, 
          alpha = 0.5) +
...
```{r}
# Visualising the Community Conservancies & Group Ranches in GAE/TCA - zoomed in
...
```{r}
#Combining the NPs & NRs in the enlarged study 

PAs.Interest.Large <- rbind(Amboseli, ChyuluHills, TsavoEast, TsavoWest, NgaiNdethia, SouthKitui)
```

```{r}
## Optional Inspection of the PAs.Interest.Large

#print(PAs.Interest.Large)
#plot(st_geometry(PAs.Interest.Large))
```

```{r}
# Next is checking RPI for NPs of Interest for Year 2000

RPI.PAs.Interest.Large.Yr.2000 <- mask(crop(RPI, PAs.Interest.Large), PAs.Interest.Large)[[1]]
```

```{r}
ggplot() +
  geom_sf(data = Kenya.Admin.Boundary, fill = NA) +
  geom_spatraster(data = RPI.PAs.Interest.Large.Yr.2000) +
  geom_sf(data = PAs.Interest.Large, fill = NA, color = "black", size = 0.5) +
  labs(title = "RPI in enlarged Protected Areas of Interest - 2000") +
  theme_beautiful()
```
...
RPI.PAs.Interest.Large.Yr.2022 <- mask(crop(RPI, PAs.Interest.Large), PAs.Interest.Large)[[23]]
```

```{r}
ggplot() +
  geom_sf(data = Kenya.Admin.Boundary, fill = NA) +
  geom_spatraster(data = RPI.PAs.Interest.Large.Yr.2022) +
  geom_sf(data = PAs.Interest.Large, fill = NA, color = "black", size = 0.5) +
  labs(title = "RPI in enlarged Protected Areas of Interest - 2022") +
  theme_beautiful()
```

```{r}
# Trying to see the change from 2000 to 2022 in the Area of Interest (GAE & TCA)

# Create the first plot for the year 2000
plot.RPI.PAs.Interest.Large.Yr.2000 <- ggplot() +
  geom_spatraster(data = RPI.PAs.Interest.Large.Yr.2000) +
  geom_sf(data = PAs.Interest.Large, fill = NA, color = "black", linewidth = 0.5) +
#  scale_fill_viridis_c(option = "plasma", name = "RPI") + # Adds a color scale
  labs(title = "RPI in enlarged Protected Areas of Interest - 2000") +
  theme_beautiful()

# Create the second plot for the year 2022
plot.RPI.PAs.Interest.Large.Yr.2022 <- ggplot() +
  geom_spatraster(data = RPI.PAs.Interest.Large.Yr.2022) +
  geom_sf(data = PAs.Interest.Large, fill = NA, color = "black", linewidth = 0.5) +
#  scale_fill_viridis_c(option = "plasma", name = "RPI") + # Adds a color scale
  labs(title = "RPI in enlarged Protected Areas of Interest - 2022") +
...
# Combine the two plots into a single output
comparison.plot.RPI.PAs.Interest.Large <- plot.RPI.PAs.Interest.Large.Yr.2000 + plot.RPI.PAs.Interest.Large.Yr.2022
```

```{r}
# Display the combined plot
print(comparison.plot.RPI.PAs.Interest.Large)
```

```{r}
# Mean RPI per Year for GAE & TCA - NPs & NRs
```

```{r}
# 4a. Extract & bind name

RPI.PAs.Interest.Large.Raw <- terra::extract(RPI, PAs.Interest.Large, fun = mean, na.rm = TRUE)

RPI.PAs.Interest.Large.Raw$NAME <- PAs.Interest.Large$NAME
```

```{r}
# 4b. Tidy

RPI.PAs.Interest.Large.TS <- as_tibble(RPI.PAs.Interest.Large.Raw) %>%
  pivot_longer(cols = starts_with("rpi."), names_to  = "Year", values_to = "Mean.RPI") %>%
  mutate(Year = as.integer(sub("rpi\\.", "", Year)))
```
...
ggplot(RPI.PAs.Interest.Large.TS, aes(x = Year, y = Mean.RPI, color = NAME)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(
    title = "Mean RPI (2000–2022) in GAE-TCA's NPs & NRs",
    x = "Year",
    y = "Mean RPI",
    color = "National Park & Reserve"
  ) +
  theme_beautiful()
```
