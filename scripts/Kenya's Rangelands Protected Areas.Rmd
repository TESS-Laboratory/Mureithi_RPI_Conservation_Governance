...
pacman::p_load(sf, terra, tidyverse, tidyterra, colorspace, ggrepel, patchwork, tmap, trend, slider, forcats)
```
...
ggplot(All.Trends.Summary, aes(x = reorder(Area, MeanSlope), y = MeanSlope)) +
  ...
  labs(
    title = "Mean RPI Trend (Theil–Sen) by Area",
    x = "Area",
    y = "Mean Slope"
  ) +
  theme_minimal() +
...
ggplot(All.Pixel.Slopes, aes(x = Area, y = Slope, fill = Area)) +
  ...
  labs(title = "Distribution of RPI Trends (Theil–Sen Slopes) by Area", x = "Area", y = "Slope") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
tm_shape(Amboseli.Slope.Raster) +
  tm_raster(
    ...
    col.legend = tm_legend(title = "Theil–Sen Slope", reverse = TRUE)) +
...
# Plotting with "geom_line()" for dashed actual means & "geom_abline()" for Theil–Sen trends
ggplot(Primary.Concern.Areas.RPI.Trends, aes(x = Year, y = MeanRPI, color = Area)) + # Dashed annual trend lines
  geom_line(size = 1, linetype = "dashed") +
  # Points
  geom_point(size = 2) +
  # Theil–Sen trend lines manually
  ...
  labs(title = "Annual Mean RPI Trends (dashed lines) with Theil–Sen Lines (straight lines)",x = "Year", y = "Mean RPI") +
  scale_color_brewer(palette = "Set1") +
  theme_beautiful() +
...
Amboseli.RPI.Mean.Filtered <- mask(Amboseli.RPI.Mean, RSQ.Mask.Amboseli, maskvalues = FALSE)

# 2. Visualising the filtered result
tmap_mode("view")
tm_shape(Amboseli.RPI.Mean.Filtered) +
  tm_raster(
    col.scale = tm_scale_continuous(values = "viridis"), 
    col.legend = tm_legend(title = "RPI (Filtered by R² ≥ 0.3)", reverse = TRUE)) +
  tm_shape(Amboseli) +
  tm_borders(col = "white", lwd = 2) +
  tm_layout(legend.outside = TRUE)
```
...
TsavoEast.RPI.Mean.Filtered <- mask(TsavoEast.RPI.Mean, RSQ.Mask.TsavoEast, maskvalues = FALSE)

# Visualising the filtered result
tmap_mode("view")
tm_shape(TsavoEast.RPI.Mean.Filtered) +
  tm_raster(
    col.scale = tm_scale_continuous(values = "viridis"), 
    col.legend = tm_legend(title = "RPI (Filtered by R² ≥ 0.3)", reverse = TRUE)) +
  tm_shape(TsavoEast) +
...
# Applying the mask to the RPI mean raster
TsavoWest.RPI.Mean.Filtered <- mask(TsavoWest.RPI.Mean, RSQ.Mask.TsavoWest, maskvalues = FALSE)

# Visualising the filtered result
tmap_mode("view")
tm_shape(TsavoWest.RPI.Mean.Filtered) +
  tm_raster(
    col.scale = tm_scale_continuous(values = "viridis"), 
    col.legend = tm_legend(title = "RPI (Filtered by R² ≥ 0.3)", reverse = TRUE)) +
  tm_shape(TsavoWest) +
...
# Applying the mask to the RPI mean raster
AET.Conservancies.RPI.Mean.Filtered <- mask(AET.Conservancies.RPI.Mean, RSQ.Mask.AET.Conservancies, maskvalues = FALSE)

# Visualising the filtered result
tmap_mode("view")
tm_shape(AET.Conservancies.RPI.Mean.Filtered) +
  tm_raster(
    col.scale = tm_scale_continuous(values = "viridis"), 
    col.legend = tm_legend(title = "RPI (Filtered by R² ≥ 0.3)", reverse = TRUE)) +
  tm_shape(AET.Conservancies) +
...
# Applying the mask to the RPI mean raster
AET.Buffers.RPI.Mean.Filtered <- mask(AET.Buffers.RPI.Mean, RSQ.Mask.AET.Buffers, maskvalues = FALSE)

# Visualising the filtered result
tmap_mode("view")
tm_shape(AET.Buffers.RPI.Mean.Filtered) +
  tm_raster(
    col.scale = tm_scale_continuous(values = "viridis"), 
    col.legend = tm_legend(title = "RPI (Filtered by R² ≥ 0.3)", reverse = TRUE)) +
  tm_shape(AET.Buffers) +
...
# Applying the mask to the RPI mean raster
AET.AllZones.RPI.Mean.Filtered <- mask(AET.AllZones.RPI.Mean, RSQ.Mask.AET.AllZones, maskvalues = FALSE)

# Visualising the filtered result
tmap_mode("view")
tm_shape(AET.AllZones.RPI.Mean.Filtered) +
  tm_raster(
    col.scale = tm_scale_continuous(values = "viridis"), 
    col.legend = tm_legend(title = "RPI (Filtered by R² ≥ 0.3)", reverse = TRUE)) +
  tm_shape(AET.AllZones) +
...
tmap_arrange(
  tm_shape(Amboseli.RPI.Mean.Filtered) +
    tm_raster(col.legend = tm_legend(title = "Amboseli (R² ≥ 0.3)")), # Updated title
  tm_shape(TsavoEast.RPI.Mean.Filtered) +
    tm_raster(col.legend = tm_legend(title = "Tsavo East (R² ≥ 0.3)")), # Updated title
  tm_shape(TsavoWest.RPI.Mean.Filtered) +
    tm_raster(col.legend = tm_legend(title = "Tsavo West (R² ≥ 0.3)")), # Updated title
  tm_shape(AET.AllZones.RPI.Mean.Filtered) +
    tm_raster(col.legend = tm_legend(title = "AET All Zones (R² ≥ 0.3)")), # Updated title
  ncol = 2
)
```

```{r}
# # Test Statistics for Filtered RPI Maps
```
...
Amboseli.Values.RPI.Rsquare <- as.data.frame(c(Amboseli.RPI.Mean, RSQ.Resampled.Amboseli), na.rm = TRUE)

# Rename for clarity
colnames(Amboseli.Values.RPI.Rsquare) <- c("RPI", "RSQ")
```
...
# Extract values from both rasters into a data.frame
TsavoEast.Values.RPI.Rsquare <- as.data.frame(c(TsavoEast.RPI.Mean, RSQ.Resampled.TsavoEast), na.rm = TRUE)

# Rename for clarity
colnames(TsavoEast.Values.RPI.Rsquare) <- c("RPI", "RSQ")
```
...
# Extract values from both rasters into a data.frame
TsavoWest.Values.RPI.Rsquare <- as.data.frame(c(TsavoWest.RPI.Mean, RSQ.Resampled.TsavoWest), na.rm = TRUE)

# Rename for clarity
colnames(TsavoWest.Values.RPI.Rsquare) <- c("RPI", "RSQ")
```
...
# Extract values from both rasters into a data.frame
AET.Conservancies.Values.RPI.Rsquare <- as.data.frame(c(AET.Conservancies.RPI.Mean, RSQ.Resampled.AET.Conservancies), na.rm = TRUE)

# Rename for clarity
colnames(AET.Conservancies.Values.RPI.Rsquare) <- c("RPI", "RSQ")
```
...
# Extract values from both rasters into a data.frame
AET.Buffers.Values.RPI.Rsquare <- as.data.frame(c(AET.Buffers.RPI.Mean, RSQ.Resampled.AET.Buffers), na.rm = TRUE)

# Rename for clarity
colnames(AET.Buffers.Values.RPI.Rsquare) <- c("RPI", "RSQ")
```
...
# Extract values from both rasters into a data.frame
AET.AllZones.Values.RPI.Rsquare <- as.data.frame(c(AET.AllZones.RPI.Mean, RSQ.Resampled.AET.AllZones), na.rm = TRUE)

# Rename for clarity
colnames(AET.AllZones.Values.RPI.Rsquare) <- c("RPI", "RSQ")
```
...
Amboseli.Values.RPI.Rsquare.Plot <- ggplot(Amboseli.Values.RPI.Rsquare, aes(x = RSQ, y = RPI)) +
...
Amboseli.Values.RPI.Rsquare.Plot
```

```{r}
# 2. TsavoEast

TsavoEast.Values.RPI.Rsquare.Plot <- ggplot(TsavoEast.Values.RPI.Rsquare, aes(x = RSQ, y = RPI)) +
  geom_hex(bins = 40) +  # increase bins for more granularity
  scale_fill_viridis_c(option = "D", trans = "log10", name = "Density") +  # log scale helps distinguish dense areas
  labs(
...
TsavoEast.Values.RPI.Rsquare.Plot
```

```{r}
# 3. TsavoWest

TsavoWest.Values.RPI.Rsquare.Plot <- ggplot(TsavoWest.Values.RPI.Rsquare, aes(x = RSQ, y = RPI)) +
  geom_hex(bins = 40) +  # increase bins for more granularity
  scale_fill_viridis_c(option = "D", trans = "log10", name = "Density") +  # log scale helps distinguish dense areas
  labs(
...
TsavoWest.Values.RPI.Rsquare.Plot
```

```{r}
# 4. AET.Conservancies

AET.Conservancies.Values.RPI.Rsquare.Plot <- ggplot(AET.Conservancies.Values.RPI.Rsquare, aes(x = RSQ, y = RPI)) +
  geom_hex(bins = 40) +  # increase bins for more granularity
  scale_fill_viridis_c(option = "D", trans = "log10", name = "Density") +  # log scale helps distinguish dense areas
  labs(
...
AET.Conservancies.Values.RPI.Rsquare.Plot
```

```{r}
# 5. AET.Buffers

AET.Buffers.Values.RPI.Rsquare.Plot <- ggplot(AET.Buffers.Values.RPI.Rsquare, aes(x = RSQ, y = RPI)) +
  geom_hex(bins = 40) +  # increase bins for more granularity
  scale_fill_viridis_c(option = "D", trans = "log10", name = "Density") +  # log scale helps distinguish dense areas
  labs(
...
AET.Buffers.Values.RPI.Rsquare.Plot
```

```{r}
# 6. AET.AllZones

AET.AllZones.Values.RPI.Rsquare.Plot <- ggplot(AET.AllZones.Values.RPI.Rsquare, aes(x = RSQ, y = RPI)) +
  geom_hex(bins = 40) +  # increase bins for more granularity
  scale_fill_viridis_c(option = "D", trans = "log10", name = "Density") +  # log scale helps distinguish dense areas
  labs(
...
AET.AllZones.Values.RPI.Rsquare.Plot
```

```{r}
# # Combining all the plots together

All.RPI.Rsquare.Plots <- 
  (Amboseli.Values.RPI.Rsquare.Plot | TsavoEast.Values.RPI.Rsquare.Plot | TsavoWest.Values.RPI.Rsquare.Plot) /
  (AET.Conservancies.Values.RPI.Rsquare.Plot | AET.Buffers.Values.RPI.Rsquare.Plot | AET.AllZones.Values.RPI.Rsquare.Plot)
