...
```{r}
# Rectifying the "Temporal_Performance_Kenya" map to remove the white break

# I shall start by determining the shared range for MAE and RMSE
...
# MAE plot with consistent color scale (using 'lajolla' - no white break)
Plot.MAE <- ggplot() +
  geom_spatraster(data = Temporal.Performance.Kenya$mae) +
  scale_fill_scico(
    palette = "lajolla",  # Yellow to red, no white break
    name = "MAE", 
    limits = shared_range,
    na.value = "grey90"
  ) +
...
# RMSE plot with same color scale as MAE
Plot.RMSE <- ggplot() +
  geom_spatraster(data = Temporal.Performance.Kenya$rmse) +
  scale_fill_scico(
    palette = "lajolla",  # Same palette as MAE
    name = "RMSE", 
    limits = shared_range,  # Same limits as MAE
    na.value = "grey90"
  ) +
  Kenya.Border +
  labs(title = "RMSE") +
...
# R² plot with appropriate scale (0 to 1, using 'batlow' - no white break)
Plot.Rsqd <- ggplot() +
  geom_spatraster(data = Temporal.Performance.Kenya$rsq) +
  scale_fill_scico(
    palette = "batlow",  # Dark to light, no white break
    name = expression(R^2), 
    limits = c(0, 1), 
    na.value = "grey90"
  ) +
