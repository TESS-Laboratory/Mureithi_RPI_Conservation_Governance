...
pacman::p_load(sf, terra, tidyverse, tidyterra, colorspace, ggrepel, patchwork, tmap, trend, slider, forcats, viridis, viridisLite, scales, hexbin, ggh4x, scico)

# DATA HANDLING#
# tidyverse -> for data manipulation, cleaning, and visualization in a cohesive framework
...
Combined.RPI.Violin.Plots <- (Amboseli.Violin.Plot + TsavoEast.Violin.Plot + TsavoWest.Violin.Plot) / (Tsavo.Ranches.Violin.Plot + Tsavo.SettledAreas.Violin.Plot + Tsavo.AllZones.Violin.Plot) / (AET.Conservancies.Violin.Plot + AET.Buffers.Violin.Plot + AET.AllZones.Violin.Plot)
...
ggsave(
  filename = "Combined_RPI_Violin_Plots.png",
...
# 1. Joining the Theil-Sen slopes to the yearly data
Combined.Trends <- Annual.RPI.Visual.Trends %>%
  left_join(All.Trends.Summary %>% select(Area, MeanSlope), by = "Area")

# 2. Creating the combined plot
Combined.Trends.Plot <- ggplot(Combined.Trends, aes(x = Year, y = MeanRPI)) +
  
  # Adding geom_ribbon to show standard deviation
  geom_ribbon(aes(ymin = MeanRPI - SD, ymax = MeanRPI + SD, fill = Area), alpha = 0.2) +
  
  # Adding geom_line for the yearly mean trend
  geom_line(aes(color = Area), linewidth = 1) +
  
  # Add geom_point for the yearly mean
  geom_point(aes(color = Area), size = 2) +
  
  # Adding geom_smooth to plot the overall Theil-Sen trend line
  geom_smooth(aes(color = Area), method = "lm", formula = y ~ x, se = FALSE, linewidth = 1.2) +
  
  # Using facets to separate areas
  facet_wrap(~ Area, ncol = 3, scales = "free_y") +
  
  # Adding labels and use a professional theme
  labs(
...
# 3. Save the plot
ggsave(
  filename = "Combined_Annual_and_TheilSen_Trends.png",
...
```{r}
# This plot provides a detailed view of the annual mean RPI trends across different study areas by combining two key analyses. Each panel represents a distinct area, with the blue line showing the **annual mean RPI** over time, allowing for the visualization of year-to-year fluctuations. The **shaded blue ribbon** around this line indicates the standard deviation, representing the variability in RPI values for each year. Overlaying these yearly trends is a **straight black line**, which shows the overall **Theil–Sen slope**. This line represents the robust, long-term trend for each area, visually summarizing whether the RPI has been increasing, decreasing, or remaining stable over the entire period. This combined visualization allows viewers to see not only short-term RPI changes but also the broader, statistically significant direction of change.
```
...
# Year (p = 0.43): No significant overall temporal effect on Mean RPI.
# Area (p < 2e-16): Highly significant differences in Mean RPI between areas.
# Year × Area (p = 0.31): No strong evidence that temporal trends differ across areas.
# Residuals: Remaining variation not explained by Year or Area.
#---

### Coefficients (Linear Model)

# Intercept (5.97): Baseline Mean RPI for the reference area at Year = 0 (not directly interpretable without centering Year).
#Year (-0.0026, ns): Slight negative overall slope, not significant.
#Area effects:

# Some areas (e.g., Tsavo East: -12.78, p = 0.067) show notably lower Mean RPI compared to the reference, borderline significant.
# Most other area coefficients are not significant.
##Year × Area interactions##

# Generally not significant.
# Tsavo East shows borderline evidence of an increasing trend (+0.0064, p = 0.067).
#---
### Model Fit

# R² = 0.479, Adjusted R² = 0.432: The model explains \~43% of the variation in Mean RPI.
#Main driver: Spatial differences between areas, not temporal change.

#---
# Mean RPI differs strongly between areas.
# There is no clear evidence of overall temporal change across years.
# Tsavo East may be an exception, with lower baseline RPI and a weak indication of recovery over time.
```
...
tmap_arrange(
  tm_shape(Amboseli.RPI.Mean.Filtered.1) +
    tm_raster(col.legend = tm_legend(title = "Amboseli Mean RPI (R² ≥ 0.1)")), 
  
  tm_shape(TsavoEast.RPI.Mean.Filtered.1) +
    tm_raster(col.legend = tm_legend(title = "Tsavo East Mean RPI (R² ≥ 0.1)")), 
  
  tm_shape(TsavoWest.RPI.Mean.Filtered.1) +
    tm_raster(col.legend = tm_legend(title = "Tsavo West Mean RPI (R² ≥ 0.1)")), 
  
  tm_shape(AET.AllZones.RPI.Mean.Filtered.1) +
    tm_raster(col.legend = tm_legend(title = "AET All Zones Mean RPI (R² ≥ 0.1)")), 
  
  tm_shape(Tsavo.AllZones.RPI.Mean.Filtered.1) +
    tm_raster(col.legend = tm_legend(title = "Tsavo All Zones Mean RPI (R² ≥ 0.1)")), 
  ncol = 2
)
```
...
Tsavo.SettledAreas.Values.RPI.Rsquare.Filtered.1.Plot <- ggplot(Tsavo.SettledAreas.Values.RPI.Rsquare.Filtered.1, aes(x = RSQ, y = RPI)) +
  ...
  scale_fill_viridis_c(option = "D", trans = "log10", name = "Density") +  # log scale helps distinguish dense areas
  labs(
    title = "RPI vs R²: Tsavo Settled Areas",
    x = expression(R^2 ~ "(Model Performance)"),
    y = "Mean RPI"
  ) +
  theme_beautiful() +
...
Tsavo.AllZones.Values.RPI.Rsquare.Filtered.1.Plot <- ggplot(Tsavo.AllZones.Values.RPI.Rsquare.Filtered.1, aes(x = RSQ, y = RPI)) +
  ...
  scale_fill_viridis_c(option = "D", trans = "log10", name = "Density") +  # log scale helps distinguish dense areas
  labs(
    title = "RPI vs R²: Tsavo All Zones",
    x = expression(R^2 ~ "(Model Performance)"),
    y = "Mean RPI"
  ) +
  theme_beautiful() +
...
Plot.Rsq.Low <- ggplot() +
  ...
  scale_fill_manual(
    name = "Model Fit",
    values = c("FALSE" = "grey90", "TRUE" = scico(5, palette = "roma")[5]),
